  ...
  (message: RuntimeMessage, sender, sendResponse) => {
    ...
    switch (message.type) {
      ...
      case "event-triggered": {
        const { event, prevMutations } = message.payload as {
          event: UserInputEvent;
          prevMutations?: MutationEvent[];
        };
          ...
          case "pre-recording": {
            if (event.eventType === "click") {
              ...
              chrome.tabs.sendMessage(
                recordedTabId,
                { type: "watch-element", payload: selector },
...
function lastElementStateDiff() {
  ...
  chrome.tabs.sendMessage(
    recordedTabId as number,
    { type: "final-diff" },
    (res) => {
      console.log(res), events.push(...res);
    }
  );
}
